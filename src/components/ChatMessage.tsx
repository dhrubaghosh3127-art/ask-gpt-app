    ...
    <div className={`w-full flex ${isUser ? 'justify-end' : 'justify-start'} px-4 my-3`}>
      <div className={`max-w-[95%] flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start`}>
        ...
        <div className={`w-full flex flex-col gap-1 ${isUser ? 'items-end' : 'items-start'}`}>
          ...
          <div className={`${isUser?'max-w-[85%] bg-blue-200 text-gray-900':'w-[99%] bg-white/90 dark:bg-gray-900/70 text-gray-900 dark:text-gray-100'} rounded-2xl px-4 py-3 shadow-sm border border-gray-200/70 dark:border-gray-800 backdrop-blur`}>
            {isEditing ? (
