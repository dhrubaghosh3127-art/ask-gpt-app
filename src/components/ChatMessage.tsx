    ...
    <div className={`w-full flex ${isUser ? 'justify-end' : 'justify-start'} px-4 my-3`}>
      <div className={`max-w-[95%] flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start`}>
        ...
        <div className={`flex-1 flex flex-col gap-1 ${isUser ? 'items-end' : 'items-start'}`}>
          ...
          <div className={`${isUser ? 'max-w-[85%] rounded-2xl bg-blue-200 text-gray-900 px-4 py-3 shadow-sm' : 'flex-1 w-full max-w-none bg-transparent text-gray-900 dark:text-white px-0 py-0'}`}>
            {isEditing ? (
