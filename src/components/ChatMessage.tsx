...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onDelete, onEdit, onRegenerate }) => {
  ...
  return (
    <div className={`w-full flex ${isUser ? 'justify-end' : 'justify-start'} px-4 my-3`}>
      <div className={`max-w-[85%] flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start`}>
        ...
        <div className={`flex-1 flex flex-col gap-1 ${isUser ? 'items-end' : 'items-start'}`}>
          ...
          <div className={`${isUser ? 'max-w-[95%] rounded-2xl bg-blue-200 text-gray-900 px-4 py-3 shadow-sm' : 'max-w-[85%] text-gray-900 dark:text-white'} leading-relaxed whitespace-pre-wrap`}>
            {isEditing ? (
